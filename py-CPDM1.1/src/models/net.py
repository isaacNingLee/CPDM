...
from src.models.resnet32 import resnet32CI
...
def parse_model_name(models_root_path, model_name, input_size,init_class_num=None):
    ...
    elif "resnet32CI" in model_name:
        base_model = ResNet32CI(models_root_path, pretrained=pretrained, create=True)
    else:
...
class ResNet32CI(Model):
    ...
    def __init__(self, models_root_path, pretrained=True, create=False):
        ...
        if not os.path.exists(self.path):
            if create:
                torch.save(resnet32CI(pretrained=pretrained), self.path)
                print("SAVED NEW RESNET32 MODEL (name=", self.name, ") to ", self.path)
            else:
                raise Exception("Not creating non-existing model: ", self.name)
        else:
            print("STARTING FROM EXISTING RESNET32 MODEL (name=", self.name, ") to ", self.path)
            print(view_saved_model(self.path))
