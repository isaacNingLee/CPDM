...
class DDGR(Method):
    ...
    def train(self, args, manager,hyperparams, test_ds_path, test_model_paths):
        ...
        model_ft, best_acc = trainDDGR.fine_tune_train_DDGR(dataset_path=manager.current_task_dataset_path,
                                                              ...
                                                              manager=manager, test_ds_path=test_ds_path, test_model_paths=test_model_paths,
                                                              )
...
class Finetune(Method):
    ...
    def inference_eval(args, manager, string_name):
        ...
        if args.class_incremental and (args.method_name in ['DDGR', 'CIDM', 'CPDM', 'GradCam']):
...
class CPDM(Method):
    ...
    def train(self, args, manager,hyperparams, test_ds_path, test_model_paths):
        ...
        model_ft, best_acc = trainCPDM.fine_tune_train_CPDM(dataset_path=manager.current_task_dataset_path,
                                                              args=args,
                                                              previous_task_model_path=manager.previous_task_model_path,
                                                              exp_dir=manager.heuristic_exp_dir,
                                                              task_counter=args.task_counter - 1,
                                                              labels_embedding=manager.dataset.labels_embedding,
                                                              labels_metric=manager.dataset.labels_metric,
                                                              batch_size=args.batch_size,
                                                              num_epochs=args.num_epochs,
                                                              lr=args.lr,
                                                              weight_decay=args.weight_decay,
