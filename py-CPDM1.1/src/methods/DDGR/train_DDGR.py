...
def train_model(args, model, criterion, optimizer, lr, dsets, batch_size, dset_sizes, use_cuda, num_epochs,
    ...
    #     n_iter = math.ceil(len(dsets['train']) / batch_size) if gen_dset is None \
    #         else math.ceil(min(len(dsets['train']), len(gen_dset['train'])) / batch_size)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            for _iter ,data in tqdm(ziploaders, desc=f'{phase} epoch {epoch + 1} (lr = {lr})'):
                ...
                if phase == 'train':
                    ...
                    optimizer.step()
                    # if 'imagenet1000' in args.ds_name:
                    #     scheduler.step(epoch + _iter / n_iter)
...
def set_lr(args, optimizer, lr, count):
    ...
    # if (count == 5) and ('cifar' in args.ds_name):
    if (count == 5):
        lr = lr * 0.1
