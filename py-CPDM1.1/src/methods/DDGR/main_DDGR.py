...
def fine_tune_train_DDGR(dataset_path, args,previous_task_model_path, exp_dir, task_counter, batch_size=200,
            ...
            if classifier_type=='self':
                ...
                while sum([s.shape[0] for s in all_images]) < num_samples:
                    ...
                    if args.num_samples is not None:
                        ...
                        # label_counter += 1
                        rnd_label = []
                        while label_counter < len(label_order_list) and len(rnd_label) < 360:
                            rnd_label += [label_order_list[label_counter] for _ in range(classifier_batch_size)]
                            label_counter += 1
                    ...
                    sample = sample_fn(
                        ...
                        (rnd_label.shape[0], 3, args.image_size, args.image_size),
                    ...
                    print(f"created {sum([s.shape[0] for s in all_images])} samples")
    ...
    if (generator_img_path_label_list is not None) and (generator_classes is not None) and (generator_class_to_idx is not None):
        # dset_loaders = {x: torch.utils.data.DataLoader(dsets[x], batch_size=batch_size, num_workers=16,
        ...
        gen_dset = combine_data_loader(generator_img_path_label_list,
                                           ...
                                           dsets)
        # gen_dset_loaders = {x: torch.utils.data.DataLoader(gen_dset[x], batch_size=batch_size, num_workers=16,
        #                                                shuffle=True, pin_memory=True, persistent_workers=True)
        #                 for x in ['train', 'val']}
    ...
    if (gen_dset is not None) and (gen_dset_sizes is not None):
