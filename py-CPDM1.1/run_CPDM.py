...
import src.framework.eval as finish_test
...
from src.methods.CPDM.diffusion.script_util import add_dict_to_argparser

parser = argparse.ArgumentParser(description='Continual Learning: CPDM')

parser.add_argument('--seed', type=int, default=0)

parser.add_argument('--model_name', type=str, default="alexnetCI")
parser.add_argument('--method_name', type=str, default="CPDM")
...
parser.add_argument('--CPDM_generator_factor', type=float, default=0.5)
...
parser.add_argument('--ddpm', default=False, action='store_true')

parser.add_argument('--diffusion_without_replay', default=False, action='store_true')
parser.add_argument('--embed_condition', type=str, choices=['default', 'no_bias', 'identity'], default='default')

parser.add_argument('--label_hid_dim', type=int, default=768)
parser.add_argument('--drop_labels_prob', type=float, default=0.5)
parser.add_argument('--w', type=float, default=0.0)
parser.add_argument('--ce_enhanced_weight', type=float, default=0.0)
parser.add_argument('--ce_uncertainty_weight', type=float, default=0.0)

parser.add_argument('--use_discriminator', default=False, action='store_true')
parser.add_argument('--unet_discriminator', default=False, action='store_true')
parser.add_argument('--discriminator_lr', type=float, default=1e-3)
parser.add_argument('--g_loss_weight', type=float, default=0.0)
parser.add_argument('--n_critic', type=int, default=1)
parser.add_argument('--random_label', default=False, action='store_true')
parser.add_argument('--wgan', default=False, action='store_true')
parser.add_argument('--wgan_gradient_penalty_weight', type=float, default=0.0)
parser.add_argument('--wgan_n_critic', type=int, default=1)
parser.add_argument('--wgan_set_weight', default=False, action='store_true')
...
parser.add_argument('--max_grad_norm', default=False, action='store_true')
...
def create_argparser_dict():
    defaults = dict(
        ...
        lr_anneal_steps=0,
        diffusion_steps=1000, # num_train_timesteps
...
def main(method=None, dataset=None):
    ...
    test_ds_paths, test_model_paths = [], []
    for task_counter in range(args.starting_task_count, args.max_task_count + 1):
        ...
        try:
            if args.no_framework:
                ...
                heuristic_single_task.framework_single_task(args, manager, test_ds_paths, test_model_paths)
    ...
    finish_test.main(args, manager, ds_paths, model_paths)
